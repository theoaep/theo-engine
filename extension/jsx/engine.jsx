/**********************************************************************************************
 * TheoEngine — ExtendScript engine (After Effects host side)
 *
...
function isGeneratedComp(c) {
    return /^TR_/.test(c.name) || c.name.indexOf("TheoEngine") === 0;
}
...
function getBeatsFolder() {
    ...
    for (var i = 1; i <= items.length; i++) {
        var it = items[i];
        if (it instanceof FolderItem && it.name === "TheoEngine beats") return it;
    }
    return items.addFolder("TheoEngine beats");
}
...
function buildReel(cfg) {
    ...
    if (intoExisting && master) {
        ...
        master = app.project.items.addComp("TheoEngine — Master", w, h, par, total, fps);
        intoExisting = false;
        startAt = 0;
    }
...
// Precomp Each — one native "Move all attributes" precomp per selected layer.
function theoReverse_precompEach() {
    try {
        var comp = activeCompOrErr();
        if (!comp) return "ERR:Open a comp first.";
        if (!comp.selectedLayers || comp.selectedLayers.length === 0) return "ERR:Select layers to precomp.";

        var selectedLayers = [], i;
        for (i = 0; i < comp.selectedLayers.length; i++) selectedLayers.push(comp.selectedLayers[i]);
        selectedLayers.sort(function (a, b) { return b.index - a.index; });

        app.beginUndoGroup("Correct Staircase Pre-comp");
        try {
            for (i = 0; i < selectedLayers.length; i++) {
                ...
                var newComp = comp.layers.precompose([layer.index], layer.name + " Comp 1", true);
                newComp.duration = oldOut - oldIn;
                if (newComp.numLayers > 0) {
                    newComp.layer(1).startTime = oldStart - oldIn;
                    newComp.layer(1).inPoint = 0;
                    newComp.layer(1).outPoint = oldOut - oldIn;
                }
                var precompLayer = comp.layer(originalIndex);
                if (precompLayer) {
                    precompLayer.startTime = oldIn;
                    precompLayer.inPoint = oldIn;
                    precompLayer.outPoint = oldOut;
                }
            }
        } finally {
            app.endUndoGroup();
        }
        return "OK:Pre-composed " + selectedLayers.length + " layer(s).";
    } catch (e) {
...
function theoReverse_build(configString) {
    ...
    try {
        ...
        app.beginUndoGroup("TheoEngine — Build Reel"); undo = true;
